...
enum SectionPage: String, CaseIterable, Identifiable {
    ...
    var subtitle: String { switch self {
        case .overview: "A clear view of your Mac, right now."
        case .cpu: "Processor activity across every logical core."
        case .gpu: "Graphics activity and available hardware readings."
        case .memory: "Unified memory, compression and swap."
        case .thermals: "Reported sensor temperatures and macOS thermal pressure."
        case .battery: "Charge, capacity and electrical measurements."
        case .power: "Modeled component power and measured battery flow."
        case .network: "Traffic on the primary network interface."
        case .storage: "System data volume capacity and physical drive activity."
        case .system: "The hardware and software behind these readings."
    } }
}
...
struct MetricCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            else if metric == .memory { Text("Pressure: \(snapshot.pressureLabel)").font(.caption).foregroundStyle(.secondary) }
            else if metric == .battery { Text(snapshot.batteryState).font(.caption).foregroundStyle(.secondary) }
            else if metric == .storage { Text("\(Metric.storageAvailable.format(snapshot[.storageAvailable])) free").font(.caption).foregroundStyle(.secondary) }
            else if metric == .cpuTemperature || metric == .gpuTemperature { Text("Reported sensor average").font(.caption).foregroundStyle(.secondary) }
            else { Text(metric == .batteryPower ? "Battery controller reading" : metric == .systemPower || metric == .packagePower ? "Experimental SMC reading" : metric.unit == "W" ? "Interval average" : "Live reading").font(.caption).foregroundStyle(.secondary) }
        }
            ...
            if points.contains(where: { $0.values[metric.rawValue] != nil }) {
                Chart {
                    ForEach(series) { point in
                        LineMark(x: .value("Time", point.date), y: .value(metric.title, point.value), series: .value("segment", point.segment))
                ...
                .frame(height: 130)
                .accessibilityLabel("\(metric.title) history, \(points.count) samples")
            } else {
                ContentUnavailableView("No readings yet", systemImage: metric.symbol, description: Text("A chart appears when this metric becomes available.")).frame(height: 130)
            }
...
struct DashboardView: View {
        ...
        NavigationSplitView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                List(SectionPage.allCases, selection: $selection) { page in
                    Label(page.rawValue, systemImage: page.symbol).tag(page).padding(.vertical, 4).accessibilityIdentifier("nav-\(page.rawValue)")
                }.listStyle(.sidebar)
                VStack(alignment: .leading, spacing: 5) {
                    Text(engine.snapshot.info["chip"] ?? "Connecting…").font(.caption.weight(.semibold))
                    Label(engine.paused ? "Paused during sleep" : "Live · every \(Int(preferences.interval))s", systemImage: "circle.fill").font(.caption2).foregroundStyle(.secondary)
                }.padding(20)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 6) { Text(page.rawValue).font(.largeTitle.weight(.semibold)); Text(page.subtitle).foregroundStyle(.secondary) }
                        Spacer()
                    ...
                    if !page.charts.isEmpty {
                        HStack { Text("Recent history").font(.title3.weight(.semibold)); Spacer(); Picker("History", selection: $historyMinutes) { Text("5 min").tag(5); Text("15 min").tag(15); Text("30 min").tag(30) }.pickerStyle(.segmented).frame(width: 230) }
                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 300), spacing: 14)], spacing: 14) {
                            ForEach(page.charts) { MetricChart(metric: $0, points: chartPoints) }
                        }
                        Text("This session · up to 900 samples / 30 minutes · gaps remain visible").font(.caption).foregroundStyle(.secondary)
                    }
    ...
    private var statusStrip: some View {
        HStack(spacing: 24) {
            Label("Thermals: \(engine.snapshot.thermalLabel)", systemImage: "thermometer.medium")
            Label("Memory: \(engine.snapshot.pressureLabel)", systemImage: "memorychip")
            Spacer()
            Text(engine.snapshot.info["interface"] ?? "Connecting…")
    ...
    @ViewBuilder private func details(_ page: SectionPage) -> some View {
        ...
        switch page {
        case .cpu:
            GroupBox("Logical cores · \(s.info["cpuClusters"] ?? "Loading topology…")") {
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 140))], spacing: 14) {
                    ForEach(Array(s.cores.enumerated()), id: \.offset) { i, value in
                        VStack(alignment: .leading) { HStack { Text("Core \(i + 1)"); Spacer(); Text(String(format: "%.0f%%", value)).monospacedDigit() }; ProgressView(value: value, total: 100) }.font(.caption).padding(8)
                    }
                }.padding(8)
            }
        ...
        case .thermals:
            GroupBox("Available temperature sensors · \(s.sensors.count)") {
                VStack(spacing: 0) { ForEach(s.sensors.keys.sorted(), id: \.self) { name in valueRow(name, String(format: "%.1f°C", s.sensors[name]!)) } }.padding(8)
            }
        ...
        case .network:
            GroupBox("Connection") { VStack { valueRow("Primary interface", s.info["interface"] ?? "—"); valueRow("Local IPv4", s.info["localIP"] ?? "—"); valueRow("Wi-Fi network", s.info["wifiSSID"] ?? "Unavailable") }.padding(8) }
        ...
        case .storage:
            GroupBox("Mounted volumes") { VStack { ForEach(s.volumes) { v in valueRow(v.name, "\(Metric.storageAvailable.format(v.available)) free of \(Metric.storageTotal.format(v.total))") } }.padding(8) }
        ...
        case .battery:
            Text("\(s.values["batteryAge"].map { "Battery controller snapshot: \(Int($0)) seconds old." } ?? "Controller timestamp unavailable.") Health is full charge capacity / design capacity; it can differ from the rounded macOS battery-health estimate. Electrical values describe the battery, not charger input or total system power.").font(.callout).foregroundStyle(.secondary)
        case .system:
            GroupBox("This Mac") { VStack { valueRow("Model", s.info["model"] ?? "—"); valueRow("Chip", s.info["chip"] ?? "—"); valueRow("macOS", s.info["os"] ?? "—"); valueRow("Hostname", s.info["hostname"] ?? "—"); valueRow("Uptime", uptime(s.values["uptime"])); valueRow("Physical memory", Metric.memoryTotal.format(s[.memoryTotal])); valueRow("Sampling cost", String(format: "%.1f ms / sample", engine.samplingMilliseconds)); valueRow("Widget sharing", engine.widgetStatus) }.padding(8) }
            GroupBox("Local by design") { Text("No accounts, backend, telemetry or analytics. History stays in memory and is cleared on quit. Only a small timestamped summary is shared with the widget on this Mac.").frame(maxWidth: .infinity, alignment: .leading).padding(12) }
        default: EmptyView()
        }
    }
    ...
    private func valueRow(_ name: String, _ value: String) -> some View { HStack { Text(name).foregroundStyle(.secondary); Spacer(); Text(value).monospacedDigit().textSelection(.enabled) }.padding(.vertical, 7) }
    private func uptime(_ seconds: Double?) -> String { guard let seconds else { return "—" }; return "\(Int(seconds)/86400)d \(Int(seconds)/3600%24)h \(Int(seconds)/60%60)m" }
}
